...
//   const [SideMenuData,setOpneSideMenuData] = useState([])
...
//     try{
//    const result = await axios.post(`${serverUrl}/api/auth/sign-out`,{withCredentials:true})
//     if(result.data){
...
//   useEffect(()=>{
//     if(currentUser){
//       setOpneSideMenuData(currentUser?.role === "admin" ? SIDE_MENU_DATA:USER_SIDE_MENU_DATA)
//     }
//     return ()=>{}
//   },[currentUser])
//   return (
//     <div className='w-64 lg:p-6 h-full flex flex-col lg:border-gray-200 '>
//      <div className='flex flex-col items-center mb-8'>
//          <div className='w-20 h-20 rounded-full bg-gray-100 overflow-hidden mb-4 border-2 border-blue-200'>
//           <img src={currentUser?.profileImageUrl || null} alt="Profile Image"
...
//             <div className='bg-blue-100 text-blue-800 text-xs font-semibold px-2.5
//             py-0.5 rounded-full mb-2'>
//               Admin
//               </div>
//           )
//          }
...
//         <button key={`menu_${index}`} className={`w-full flex items-center gap-4 text-[15px] rounded-xl
//           ${activeMenu === item.label?"text-blue-500 bg-linear-to-r from-blue-50/40 to-blue-100/50": ""}
...
function SideMenu({ activeMenu }) {
  ...
  const handleLogOut = async () => {
    try {
      ...
      if (result.data) {
        dispatch({ type: 'SIGN_OUT_SUCCESS' }) // Replace with your actual action
        navigate("/login")
      }  
    } catch (err) {
